-- Password-only HR gate: one shared auth user (synthetic email).
-- Must match src/lib/auth/general-hr.ts (GENERAL_HR_AUTH_EMAIL).
-- Applied by `supabase db push` on remote; local `db reset` also runs seed.sql (same user, idempotent).
