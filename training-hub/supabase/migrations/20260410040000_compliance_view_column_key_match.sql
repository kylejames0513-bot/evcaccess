-- Fix compliance view: look up completions by column_key, not just
-- training_type_id. This lets Initial Med Training (id=5) completions
-- satisfy the Med Recert (id=4) requirement since both share
-- column_key='MED_TRAIN'.
--
-- Also compute effective_expiration from the required training's
-- renewal_years so cross-type completions get the right renewal cycle.
-- e.g. If Initial Med (renewal_years=0) completion is found for a
-- Med Recert (renewal_years=3) requirement, expiration = completion + 3yr.
...
SELECT
  ...
  -- Show "Initial Med Training" when Med Recert is required but
  -- the employee has no prior med completion at all.
  CASE
  ...
  -- Use the required type's renewal_years to compute expiration for
  -- cross-type completions. Falls back to the record's own expiration
  -- for same-type or renewal_years=0.
  CASE
  ...
  -- 90/60/30/overdue tier columns (using computed expiration)
  CASE
    WHEN latest.completion_date IS NOT NULL AND tt.renewal_years > 0
...
-- Look for the latest completion across ALL training types sharing the
-- same column_key. This lets Initial Med Training (MED_TRAIN) satisfy
-- a Med Recert (MED_TRAIN) requirement.
LEFT JOIN LATERAL (
...
COMMENT ON VIEW employee_compliance IS
  ...
  'Expiration computed from the required training renewal_years.';
