...
export default function TrainingCatalogPage() {
  ...
  if (loading) return <div className="p-6 text-slate-400">Loading training catalog...</div>;
    ...
    <div className="max-w-5xl mx-auto">
      ...
      {editing && (
        <div className="bg-white rounded-xl border-2 border-blue-200 p-5 mb-6">
          <h2 className="font-semibold text-slate-900 mb-3">{editing.id === null ? "Add new training" : `Edit: ${editing.name}`}</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-4">
            <label className="block">
              ...
              <input type="text" value={editing.name} onChange={e => setEditing({ ...editing, name: e.target.value })}
                className="mt-1 block w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white" placeholder="e.g. CPR/FA" />
            </label>
            <label className="block">
              <span className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Column key (for imports)</span>
              <input type="text" value={editing.column_key} onChange={e => setEditing({ ...editing, column_key: e.target.value })}
                className="mt-1 block w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white" placeholder="e.g. CPR" />
            </label>
            <label className="block">
              <span className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Renewal period (years, 0 = one time)</span>
            ...
            <label className="block">
              <span className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Class capacity</span>
          ...
          <div className="flex gap-2">
            ...
            <button type="button" onClick={() => { setEditing(null); setError(null); }}
              className="px-4 py-2 border border-slate-200 bg-white rounded-lg hover:bg-slate-50 text-sm text-slate-900">Cancel</button>
          </div>
        </div>
      )}
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        <table className="min-w-full text-sm">
          <thead className="bg-slate-50">
            <tr>
              <th className="px-3 py-2 text-left w-8 border-b border-slate-100"></th>
              <th className="px-3 py-2 text-left text-[11px] font-semibold uppercase tracking-wider text-slate-400 border-b border-slate-100">Name</th>
              <th className="px-3 py-2 text-left text-[11px] font-semibold uppercase tracking-wider text-slate-400 border-b border-slate-100">Key</th>
              <th className="px-3 py-2 text-center text-[11px] font-semibold uppercase tracking-wider text-slate-400 border-b border-slate-100">Renewal</th>
              <th className="px-3 py-2 text-center text-[11px] font-semibold uppercase tracking-wider text-slate-400 border-b border-slate-100">Capacity</th>
              <th className="px-3 py-2 text-center text-[11px] font-semibold uppercase tracking-wider text-slate-400 border-b border-slate-100">Aliases</th>
              <th className="px-3 py-2 text-center text-[11px] font-semibold uppercase tracking-wider text-slate-400 border-b border-slate-100">Status</th>
...
function TrainingRow({ tt, expanded, onToggleExpand, onEdit, onToggleActive, newAlias, onNewAliasChange, onAddAlias, onRemoveAlias }: {
  ...
  return (
    <>
      <tr className={`hover:bg-slate-50 ${!tt.is_active ? "opacity-50" : ""}`}>
        <td className="px-3 py-2">
          <button type="button" onClick={onToggleExpand} className="text-slate-400 hover:text-slate-600">
            {expanded ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}
          </button>
        </td>
        <td className="px-3 py-2 font-medium text-slate-900">
          <Link href={`/trainings/${tt.id}`} className="text-blue-600 hover:text-blue-800">{tt.name}</Link>
        </td>
        <td className="px-3 py-2 font-mono text-xs text-slate-500">{tt.column_key}</td>
        <td className="px-3 py-2 text-center text-slate-900">{tt.renewal_years > 0 ? `${tt.renewal_years} yr` : "Once"}</td>
        <td className="px-3 py-2 text-center text-slate-900">{tt.class_capacity}</td>
        <td className="px-3 py-2 text-center text-slate-900">{tt.aliases.length}</td>
        <td className="px-3 py-2 text-center">
          <button type="button" onClick={onToggleActive}
            className={`px-2 py-0.5 rounded-md text-xs font-medium ${tt.is_active ? "bg-green-100 text-green-800" : "bg-slate-100 text-slate-500"}`}>
            {tt.is_active ? "Active" : "Inactive"}
          </button>
        </td>
        <td className="px-3 py-2 text-right">
          <button type="button" onClick={onEdit} className="text-slate-400 hover:text-blue-600">
        ...
        <tr className="bg-slate-50">
          ...
          <td colSpan={7} className="px-3 py-3">
            ...
            <div className="flex flex-wrap gap-2 mb-2">
              {tt.aliases.map((a, i) => (
                <span key={i} className="inline-flex items-center gap-1 bg-white border border-slate-200 rounded-md px-2 py-1 text-xs text-slate-900">
                  {a}
                  <button type="button" onClick={() => onRemoveAlias(a)} className="text-slate-400 hover:text-red-500">
                    <X className="h-3 w-3" />
                  </button>
                </span>
              ))}
              {tt.aliases.length === 0 && <span className="text-xs text-slate-400">No aliases yet. Add one so imports from Paylocity/PHS/Access can match this training.</span>}
            </div>
            <div className="flex gap-2">
              <input type="text" value={newAlias} onChange={e => onNewAliasChange(e.target.value)}
                placeholder="Add alias (e.g. 'med training', 'CPR.FA')..." className="px-3 py-2 border border-slate-200 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white w-64"
                onKeyDown={e => { if (e.key === "Enter") onAddAlias(); }} />
              <button type="button" onClick={onAddAlias} disabled={!newAlias.trim()}
                className="px-3 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-xs font-semibold disabled:opacity-50">Add</button>
            </div>
          </td>
        </tr>
      )}
