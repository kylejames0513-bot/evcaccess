...
function writeAuditReport(ss, issues) {
  var NAVY = COLORS.NAVY;
  var RED = COLORS.RED;
  var ORANGE = COLORS.ORANGE;
  var GREEN = COLORS.GREEN;
  var BLUE = COLORS.BLUE;
  var LIGHT_GRAY = COLORS.LGRAY;
...
function fixTrainingSheetIssues() {
  ...
  for (var r = 1; r < data.length; r++) {
    ...
    for (var c = 3; c < data[r].length; c++) {
      ...
      // FX variants → standardize via shared isFailureCode/standardizeFailureCode
      if (isFailureCode(s)) {
        var stdFail = standardizeFailureCode(s);
        if (stdFail) {
          sheet.getRange(r + 1, c + 1).setValue(stdFail);
          fixedFail++;
        }
        continue;
      }
  ...
  for (var r = 1; r < data.length; r++) {
    ...
    if (medCol >= 0 && pmCol >= 0) {
      ...
      // If either has a failure code — leave it alone
...
// isExcusal(), parseCellDate(), and formatAuditDate() are now in Shared.gs
